...
def expand_words(img,kernal_size):
    dilate_kernel = np.zeros((kernal_size,kernal_size),np.uint8)
    dilate_kernel[int(kernal_size/2),:] = np.ones((1,kernal_size),np.uint8)
...
def getDetBoxes_core(text_map, link_map, text_threshold, link_threshold, low_text):
    ...
    ###########################################################################
    # You can choose the right level for your detection
    # You can simply annotate the other two levels' codes 
    #                                             --LDF
    ...
    # row level
    text_score_comb = np.clip(text_score + link_score, 0, 1)
    text_score_comb = expand_words(text_score_comb,5)
